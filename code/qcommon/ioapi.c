...
#define QL_XOR_SUPPORT 1

#ifdef QL_XOR_SUPPORT
...
#ifdef QL_XOR_SUPPORT
static size_t fread_xor (void *ptr, size_t size, size_t nmeb, FILE *stream)
{
	size_t r;
	long pos;
	long bytesRead;
   ...
   uLong size;
{
    uLong ret;
#ifdef QL_XOR_SUPPORT
    ret = (uLong)fread_xor(buf, 1, (size_t)size, (FILE *)stream);
#else
