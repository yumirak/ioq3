...
static void Cmd_VoiceTell_f( gentity_t *ent, qboolean voiceonly ) {
	...
	if ( trap_Argc () < 3 ) {
		trap_SendServerCommand( ent-g_entities, va( "print \"Usage: %s <player id> <voice id>", voiceonly ? "votell" : "vtell" ) );
		return;
	}
...
void Cmd_GameCommand_f( gentity_t *ent ) {
	...
	if ( trap_Argc() != 3 ) {
		trap_SendServerCommand( ent-g_entities, va( "print \"Usage: gc <player id> <order 0-%d>", numgc_orders - 1 ) );
		return;
	}
	...
	if ( order < 0 || order >= numgc_orders ) {
		trap_SendServerCommand( ent-g_entities, va("print \"Bad order: %i", order));
		return;
	}
...
void Cmd_CallVote_f( gentity_t *ent ) {
	...
	trap_SendServerCommand( -1, va("print \"%s called a vote.", ent->client->pers.netname ) );
...
void Cmd_CallTeamVote_f( gentity_t *ent ) {
		...
		if ( !arg2[0] ) {
			...
			if ( i >= 3 || !arg2[i]) {
				...
				if ( i < 0 || i >= level.maxclients ) {
					trap_SendServerCommand( ent-g_entities, va("print \"Bad client slot: %i", i) );
					return;
				}

				if ( !g_entities[i].inuse ) {
					trap_SendServerCommand( ent-g_entities, va("print \"Client %i is not active", i) );
					return;
				}
			}
			else {
				...
				if ( i >= level.maxclients ) {
					trap_SendServerCommand( ent-g_entities, va("print \"%s is not a valid player on your team.", arg2) );
					return;
				}
			}
		}
	...
	for ( i = 0 ; i < level.maxclients ; i++ ) {
		if ( level.clients[i].pers.connected == CON_DISCONNECTED )
			continue;
		if (level.clients[i].sess.sessionTeam == team)
			trap_SendServerCommand( i, va("print \"%s called a team vote.", ent->client->pers.netname ) );
	}
...
/*
	...
	//trap_SendServerCommand( ent-g_entities, va("print \"visited %d of %d areas", n, max));
	trap_SendServerCommand( ent-g_entities, va("print \"%d%% level coverage", n * 100 / max));
*/
}
...
void ClientCommand( int clientNum ) {
	...
	if (Q_stricmp (cmd, "give") == 0)
		...
		trap_SendServerCommand( clientNum, va("print \"unknown cmd %s", cmd ) );
}
