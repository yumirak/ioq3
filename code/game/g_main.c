...
//#endif
	...
	{ &pmove_FlyAccel, "pmove_FlyAccel", "8", CVAR_SYSTEMINFO, 0, qfalse},
...
void SetLeader(int team, int client) {
	...
	if ( level.clients[client].pers.connected == CON_DISCONNECTED ) {
		PrintTeam(team, va("print \"%s is not connected", level.clients[client].pers.netname) );
		return;
	}
	if (level.clients[client].sess.sessionTeam != team) {
		PrintTeam(team, va("print \"%s is not on the team anymore", level.clients[client].pers.netname) );
		return;
	}
	for ( i = 0 ; i < level.maxclients ; i++ ) {
		if (level.clients[i].sess.sessionTeam != team)
	...
	PrintTeam(team, va("print \"%s is the new team leader", level.clients[client].pers.netname) );
}
